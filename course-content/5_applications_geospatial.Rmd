...
Look at the details in spark:
Make sure that you do not see a `Broadcast Hash Join`
![actual join](img/sort-join.png)
Instead of the `BroadcatHashJoin` above, you should see a `RangeJoin`, if everything is set up correctly:
![desired join](img/range-join.png)

Why is the `BroadcatHashJoin` bad? It will not scale, as catalyst optimizations are not used to improve upon $O(n^2)$.
...
But below finally works nicely (if you prefer SQL over tidyverse approach):
```{r}
...
The last approach where the code actually works as expected for an R user's perspective and only calls out to SQL where absolutely required.
