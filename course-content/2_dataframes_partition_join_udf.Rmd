...
In the first notebook we already looked into JOINs. Let us revisit them.
...
> It is important to tune `spark.sql.autoBroadcastJoinThreshold` parameter. By default only dataframes up to 10MB are broadcasted. But do not forget to increase the memory of the executors and driver accordingly to what you want to broadcast.
...
> Arrow makes the data handover faster, but seems to be limited in functionality.
> Simply do not load the R Arrow package when starting the spark session.
...
Look at the time spent in the calculations and the execution graph. On my computer this is about 3 seconds vs. 80ms in favour of the native code.

![UDF performance vs. native code](img/udf-vs-native.png)
...
sparkR offers `spark_apply` to handle working with groups which easily allows to scale out computation i.e. paralellize it per each group. Ad it is by far simpler and nicer to use.

### without spark

Go back to the beginning [recap-3tidyverse_intro.Rmd secton Managing many models](course-content/recap-3tidyverse_intro.Rmd).
...
Examine the (nested) schema.

You need an additional package `sparklyr.nested` to make it work.

```{r}
my_json <- spark_read_json(spark, "datasets/json_sample")
my_json %>%
  head
